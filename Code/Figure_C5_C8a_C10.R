...
d = data.frame(dN=yeast$dN, dS=yeast$dS, dSadj=yeast$dSadj, dNdSadj=yeast$dNdSadj, dNdS=yeast$dNdS, cai=yeast$cai, len=yeast$len, expr=yeast$expr, disp=yeast$disp,cov=yeast$coverage)
...
#yeast = read.table(input.file, head=T, sep="\t")
#d = data.frame( small_dSadj=yeast$small_dSadj, small_dNdSadj=yeast$small_dNdSadj, small_dNdS=yeast$small_dNdS, sacCer_sacPar_dN=yeast$sacCer_sacPar_dN, sacCer_sacPar_dS=yeast$sacCer_sacPar_dS, cai=yeast$cai, len=yeast$len, expr=yeast$expr, disp=yeast$disp,cov=yeast$coverage)
#rownames(d)=yeast$orf
#d = na.omit(d)
...
for (i in 1:length(depVars)){
	...
	for (j in 1:ncol(pc.mat)){
		...
		logVar2 = log(d[,rate]+dNadd)
		logVar1 = pc.values[,j] #don't log transform...often has negative numbers
		...
		plot.file = paste("R-base_PCA_plots/",output.prefix,"_",tempID,".png",sep="")
		png(plot.file)
		plot(logVar1,logVar2,
			xlab=paste("ln(",rate,"+",dNadd,")",sep=""), ylab = paste("PC",j,sep=""),
...
stat.file = paste("R-base_PCA_plots/",output.prefix,"_PCR_stats.txt",sep="")
...
variance.file = paste("R-base_PCA_plots/",output.prefix,"_R-Base_prcomp_percent_variance_explained.png",sep="")
png(variance.file)
...
pca.text.file = paste("R-base_PCA_plots/",output.prefix,"_R-Base_prcomp_pca_values.txt",sep="")
...
png(paste("R-base_PCA_plots/",output.prefix,"_PC1_vs_PC2.png",sep=""))
par(mar=c(5,5,3,10))
...
colLab = function(n, labelColors, clusMember) { 
   ...
   n 
	...
	cluster.file = paste("R-base_Clustering_plots/Pearson_Dissimilarity_Complete_",output.prefix,"_Hierarchical_Clustering_na_omit.png",sep="")
		cor.mat = cor(as.matrix(d))
		dis.mat = 1 - cor.mat
